...
export default function WelcomePage() {
  ...
  const cityYPos = useTransform(scrollY, [0, 200], [0, -100]);
  const cityOpacity = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.5, 0]);

  const heroYPos = useTransform(scrollY, [0, 200], [0, -150]);
  const heroOpacity = useTransform(scrollY, [0, 300, 500], [1, 1, 0]);

  const textYPos = useTransform(scrollY, [0, 200, 300, 500], [0, 50, 50, 300]);
  const textScale = useTransform(scrollY, [0, 300], [1, 1.5]);

  return (
    <>
      <header id="welcome-header">
        <motion.div id="welcome-header-content" style={{ scale: textScale, y: textYPos }}>
        ...
        <motion.img
          style={{ opacity: cityOpacity, y: cityYPos }}
          src={cityImg}
          alt="A city skyline touched by sunlight"
          id="city-image"
        />
        <motion.img
          style={{ opacity: heroOpacity, y: heroYPos }}
          src={heroImg}
